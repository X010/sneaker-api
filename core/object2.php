...
//			}
    ...
    public function create_batch($db_datas, $createtime = 1){
        ...
        if ($db_datas){
            ...
            $ret = $this->app->db2->insert($this->tablename, $db_datas);
        }
        if ($ret === False){
            if (stripos($this->app->db2->pdo->errorInfo()[2], 'Duplicate entry') === 0){ //记录已存在
                error(1200);
            } else {
                error(9900);
            }
	...
	public function create($db_data, $createtime=1){
        ...
        if ($db_data){
            ...
            $ret = $this->app->db2->insert($this->tablename, $db_data);
        }
        if ($ret === False){
            if (stripos($this->app->db2->pdo->errorInfo()[2], 'Duplicate entry') === 0){ //记录已存在
                error(1201);
	...
	public function update($db_set, $db_where, $allow_all=false, $updatetime = True){
        ...
        if ($db_set){
            ...
            $ret = $this->app->db2->update($this->tablename, $db_set, $db_where);
        }
	...
	public function read($db_fields = '*', $db_where = NULL){
		$ret = $this->app->db2->select($this->tablename, $db_fields, $db_where);
        $ret === False && error(9900);
        foreach($ret as $k=>$v){
	...
	public function delete($db_where){		
		$ret = $this->app->db2->delete($this->tablename, $db_where);
        $ret === False && error(9900);
        return $ret;
	}
	...
	public function count($db_where = NULL){		
		$ret = $this->app->db2->count($this->tablename, $db_where);
        $ret === False && error(9900);
        return $ret;
	}
	...
	public function has($db_where = NULL){
		...
		$ret = $this->app->db2->has($this->tablename, $db_where);
        return $ret;
	}

    public function sum($db_fields, $data = NULL){
        $db_where = $this->build_query($data, False);
        $ret = $this->app->db2->sum($this->tablename, $db_fields, $db_where);
    ...
    public function get_code2(){
        ...
        $res = $this->app->db2->select($this->tablename, '*', $db_where);
        if($res){
            $code = $res[0]['code']+1;
        } else {
    ...
    public function get_name_by_id($tablename, $id){
        $res = $this->app->db2->select($tablename, '*', ['id'=>$id]);
        if(!$res){
            error(1210);
        }
        return $res[0]['name'];
    ...
    public function get_names_by_ids($tablename, $ids){
        ...
        $names = [];
        $res = $this->app->db2->select($tablename, '*', ['id'=>$id_list2]);
        $res2 = [];
        foreach($res as $val){
