...
class SettleCustomer extends Object{

	/**
	 * 入库所需字段（必须），如果加星号，代表可以插入但是不可以修改
	 */
	protected $format_data = ['*cid','*cname','*ccid','*ccname','stock_list','uid','uname','cuid', 'cuname','checktime',
		'status','amount_price','tax_price','negative_id','memo','pay_type','discount_price','settle_type','small_amount'];
	...
	protected $amount_data = ['amount_price','tax_price','discount_price','small_amount'];
	...
	public function __construct($id = NULL){
		parent::__construct('b_settle_customer', $id);
	}
	...
	public function my_create($data){
		...
		$discount_price = get_value($data, 'discount_price', 0);

//		if($amount_price < 0){
//			error(3406);
//		}

		if($discount_price > $amount_price && $amount_price>0){
			error(3405);
		}
		$data['amount_price'] = price_sub($amount_price, $discount_price);

		if($amount_price == 0){
			$data['tax_price'] = 0;
		}
		else{
			$discount = $data['amount_price']/$amount_price;
			$data['tax_price'] = $tax_price*$discount;
		}
		...
		$sg_model -> my_create($id, $data['stock_list'], 2);
...
//		}
		...
		//写明细表
		$sg_model = new SettleGlist();
		$sg_model -> my_update($this->id, $data['stock_list'], 2);
	 ...
	 * @return int 结算单号
	 */
	public function my_check($data, $type, $settle_type = Null){
		$data['status'] = 2;
		...
		if($settle_type){
			$db_set['settle_type'] = $settle_type;
		}
	...
	public function my_read(){
		...
		$tax_group = $sg_model->get_tax_group($this->id, 1);
		$temp_price = price_add($res[0]['amount_price'], $res[0]['discount_price']);
		//var_dump($temp_price);
		$discount = $res[0]['amount_price']/$temp_price;
		//var_dump($discount);
		foreach($tax_group as $key=>$val){
			$tax_group[$key]['tax_price'] = format_yuan($discount*$tax_group[$key]['tax_price']);
			$tax_group[$key]['discount_price'] = format_yuan((1-$discount)*$tax_group[$key]['amount_price']);
			$tax_group[$key]['amount_price'] = format_yuan($discount*$tax_group[$key]['amount_price']);
		}
	...
	public function read_stock($data){
		...
		//$stock_id = get_value($data, 'stock_id');
		$result = [];
		$si_model = new StockIn();
		$so_model = new StockOut();
		...
		if($order_id){
			if($order_id[0] == 1){
				...
				if($limit){
					$param['page'] = 1;
					$param['page_num'] = $limit;
				}

				$res = $si_model -> read_list($param);
				if($res['count']){
					foreach($res['data'] as $key=>$val){
						$res['data'][$key]['stock_type'] = 2;
					}
				}
				$result = $res['data'];
				//再找进货的单据
			...
			//如果是根据条件来检索
			param_need($data, ['ccid','begintime','endtime']); //必选

			//先找退货的单据
			//条件1-本公司单据 2-客户 3审核时间在时间段内 4-未结算过的，已审核的 5-必须是退货单
			$si_data = [
				'cid'=>$this->app->Sneaker->cid,
				'out_cid'=>$data['ccid'],
				'status'=>2,
			...
			//再找进货的单据
			//条件1-本公司单据 2-客户 3审核时间在时间段内 4-结算日在基准日之前 5-未结算过的，已审核的 6-必须是采购单
			$so_data = [
				'cid'=>$this->app->Sneaker->cid,
				'in_cid'=>$data['ccid'],
				'status'=>[3,4],
	...
	public function my_flush(){
		...
		foreach($stock_list as $stock){
			if($stock[0] == '2'){
				...
				if($so_res[0]['commission_status'] == 1){
					error(3409);
				}
			}
		}
		...
		foreach($stock_list as $stock){
			if($stock[0] == '2'){
				...
				if($order_id){
					$o_res = $o_model->read_by_id($order_id);
					if($o_res && $o_res[0]['mall_orderno']){
						$mall_model->notice_order($o_res[0]['mall_orderno'], 10, $msg);
					}
				}
			}
		}
