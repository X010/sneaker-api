...
class StoreGoods_Test extends PHPUnit_Framework_TestCase{
    private $data = [];
    private $id = 0;
    private $uid = [];
    private $ticket = [];
    ...
    public function test_all(){
        ...
        //创建仓库商品
        $this->create_store_goods();
        //读取入库商品清单
        $this->read_in_store_goods();
        //第二个账号创建仓库商品
        $this->create_store_goods2();
        //第二个账号向第一个账号授权
        $this->create_customer();
        //读取出库商品清单
        $this->read_out_store_goods();
        //查看仓库商品是否存在
        $this->exists_store_goods1();
        //设置仓库商品价格、供应商
        $this->update_store_goods();
        //读取仓库价格
        $this->read_store_goods();
        //读取仓库价格列表
        $this->read_store_goods2();
        //取消仓库价格
        $this->delete_store_goods();
        //查看仓库商品是否存在
        $this->exists_store_goods2();
    }
    ...
    public function create_store_goods(){
    ...
    public function create_store_goods2(){
        $url = get_api_host() . "/store_goods/create";
        $data = [[
    ...
    public function read_in_store_goods(){
        $url = get_api_host() . "/store_goods/read_in";
        $data = [
            'in_sid' => $this->sid['unittest1'],
            'ticket' => $this->ticket['unittest1']
        ];
    ...
    public function read_out_store_goods(){
        $url = get_api_host() . "/store_goods/read_out";
        $data = [
            'in_cid' => $this->cid['unittest1'],
    ...
    public function exists_store_goods1(){
        $url = get_api_host() . "/exists/store_goods";
        $this->data = [
            'ticket' => $this->ticket['unittest1'],
            'gid' => $this->gid['g1'],
            'in_sid' => $this->sid['unittest1']
    ...
    public function exists_store_goods2(){
        $url = get_api_host() . "/exists/store_goods";
        $this->data = [
            'ticket' => $this->ticket['unittest1'],
            'gid' => $this->gid['g1'],
    ...
    public function update_store_goods(){
        $url = get_api_host() . "/store_goods/update/". $this->sgid;
        $data = [
    ...
    public function read_store_goods(){
        $url = get_api_host() . "/store_goods/read/";
        $data = [
            'in_sid' => $this->sid['unittest1'],
        ];
        $data['ticket'] = $this->ticket['unittest1'];
    ...
    public function read_store_goods2(){
        $url = get_api_host() . "/store_goods/read/". $this->sgid;
        $data['ticket'] = $this->ticket['unittest1'];
        $res = curl_post($url, $data);
        $ret = json_decode($res, True);
        if (!$ret || $ret['err'] != 0){var_dump($res);}
    ...
    public function delete_store_goods(){
        $url = get_api_host() . "/store_goods/delete/". $this->sgid;
        $data['ticket'] = $this->ticket['unittest1'];
        $res = curl_post($url, $data);
        $ret = json_decode($res, True);
        if (!$ret || $ret['err'] != 0){var_dump($res);}
