...
class medoo
{
	...
	public function query($query)
	{
		if ($this->debug_mode)
		{
			...
			$msg = str_replace('"', '`', $query)."\n"; //modify by Linvo 20150916
			debug_log('['. $app->Sneaker->imark. ']  '. $msg);

			//$this->debug_mode = false;

			//return false; //modify by Linvo 20150916
		}
	...
	protected function data_implode($data, $conjunctor, $outer_conjunctor = null)
	{
		...
		foreach ($data as $key => $value)
		{
			...
			{
				$temp = 0 !== count(array_diff_key($value, array_keys(array_keys($value))));
				$wheres[] = $temp?
	...
	protected function select_context($table, $join, &$columns = null, $where = null, $column_fn = null)
	{
		...
		if (isset($column_fn))
		{
			...
			else
			{
				...
				//modified by fish begin
				if (is_array($columns)){
					$column = '';
					foreach($columns as $val){
						$column .= $column_fn . '(' . $this->column_push($val) . ") as $val,";
					}
	...
	public function insert($table, $datas)
	{
		...
		foreach ($datas as $data)
		{
			...
			$v[] = '(' . implode($values, ', ') . ')'; //modify by Linvo@20151020

		}
		$res = $this->exec('INSERT INTO "' . $table . '" (' . implode(', ', $columns) . ') VALUES '. implode(', ', $v)); //modify by Linvo@20151020
		$lastId[] = $res ? $this->pdo->lastInsertId() : $res; //modify by Linvo@20151020
	...
	public function sum($table, $join, $column = null, $where = null)
	{
		...
		return $query ? $query->fetch(
			(is_string($column) && $column != '*') ? \PDO::FETCH_COLUMN : \PDO::FETCH_ASSOC
		) : false;
	}

	public function distinct($table, $join, $column = null, $where = null)
	{
		$query = $this->query($this->select_context($table, $join, $column, $where, 'DISTINCT'));
